...
std::string WrapWithEvidenceEnvelope(const std::string& json, uint64_t recordId,
    ...
    const size_t closingBrace = json.rfind('}');
    if (closingBrace == std::string::npos) return json;

    std::ostringstream out;
    // Keep the behavior payload first for humans tailing JSONL.  Evidence
    // identity remains top-level and machine-compatible, but no longer hides
    // the application/action/path/network fields behind bookkeeping metadata.
    out << json.substr(0, closingBrace)
        << ",\"record_id\":" << recordId
        << ",\"session_id\":\"" << sessionId << "\""
        << ",\"source_file\":\"" << sourceFileName << "\""
        << ",\"byte_offset\":" << byteOffset
        << ",\"content_hash\":\"" << Fnv1a64Hex(json) << "\"}"
        << json.substr(closingBrace + 1);
    return out.str();
}
