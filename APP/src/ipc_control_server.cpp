...
// Extracts "key":["a","b","c"] -- a flat array of strings. No nested
// objects/escaping support needed: application executable names never
// contain '"' or ']' in practice, matching this project's existing
// "simple identifiers only" convention for the hand-rolled extractors.
bool ExtractJsonStringArray(const std::string& line, const std::string& key,
    ...
    size_t cursor = pos + 1;
    while (cursor < end) {
        const auto quoteStart = line.find('"', cursor);
        if (quoteStart == std::string::npos || quoteStart > end) break;
        const auto quoteEnd = line.find('"', quoteStart + 1);
        if (quoteEnd == std::string::npos || quoteEnd > end) break;
        out.push_back(line.substr(quoteStart + 1, quoteEnd - quoteStart - 1));
        cursor = quoteEnd + 1;
    }
    return true;
}
...
IpcControlServer::IpcControlServer(AppLogMonitor& monitor, std::function<void()> shutdownCallback)
...
std::string IpcControlServer::Dispatch(const std::string& requestJson, void* pipeHandle) {
    ...
    const bool isMutating = (command == "StartMonitoring" || command == "StopMonitoring" ||
        command == "SetPersistence" || command == "SetWatchlist" ||
        command == "SetRetentionBudget" || command == "Shutdown");
    bool staleRejected = false;
    if (isMutating) {
        std::string expectedSession;
        if (ExtractJsonString(requestJson, "expected_session_id", expectedSession) &&
            expectedSession != AppLogLogger::Instance().GetSessionId()) {
            ok = false;
    ...
    uint64_t acceptedWatchlistRevision = 0;
    bool watchlistApplied = false;

    if (staleRejected) {
        // fall through to audit below
    }
        ...
        if (!ExtractJsonBool(requestJson, "enabled", enabled)) {
            ...
            AppLogLogger::Instance().SetSaveLogsEnabled(enabled);
            revision_.fetch_add(1, std::memory_order_relaxed);
        }
    }
    else if (command == "SetWatchlist") {
        std::vector<std::string> apps;
        if (!ExtractJsonStringArray(requestJson, "applications", apps)) {
            ok = false;
            error = "Missing or invalid 'applications' string array.";
        } else {
            acceptedWatchlistRevision = monitor_.ApplyWatchlistRevisioned(apps);
        ...
        if (!ExtractJsonUint64(requestJson, "budget_bytes", budgetBytes) || budgetBytes == 0) {
            ...
            auto& logger = AppLogLogger::Instance();
        ...
        auto lines = AppLogLogger::Instance().GetRecentLines();
    ...
    if (watchlistApplied) {
        extra += ",\"accepted_watchlist_revision\":" + std::to_string(acceptedWatchlistRevision);
    }

    // Always return the native effective watchlist, including on GetStatus. The GUI can now
    // recover after a restart and render collector truth without treating watchlist.txt as an
    // authoritative live-control channel.
    {
        const auto names = monitor_.WatchlistNames();
        std::ostringstream watchlistJson;
        watchlistJson << ",\"watchlist\":[";
        for (size_t i = 0; i < names.size(); ++i) {
            if (i) watchlistJson << ",";
            watchlistJson << "\"" << JsonEscape(names[i]) << "\"";
        }
        watchlistJson << "]";
        extra += watchlistJson.str();
    }
    ...
    if (isMutating) {
        ...
        audit << "{\"t_unix_ms\":" << now_ms << ",\"endpoint\":\"application\","
        ...
        AppLogLogger::Instance().Write(audit.str());
    }
    ...
    out << "{\"proto_version\":1,\"request_id\":\"" << JsonEscape(requestId) << "\","
        ...
        << "\"watchlist_revision\":" << monitor_.CurrentWatchlistRevision() << ","
        << "\"monitoring_enabled\":" << (monitor_.IsMonitoringEnabled() ? "true" : "false") << ","
        << "\"save_logs_enabled\":" << (AppLogLogger::Instance().IsSaveLogsEnabled() ? "true" : "false")
        << extra
        << "}";
    return out.str();
}
