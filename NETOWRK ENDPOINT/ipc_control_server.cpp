...
IpcControlServer::IpcControlServer(NetworkMonitor& monitor, AsyncLogger& logger,
...
std::string IpcControlServer::Dispatch(const std::string& requestJson, void* pipeHandle) {
    ...
    if (isMutating) {
        ...
        audit << "{\"t_unix_ms\":" << now_ms << ",\"endpoint\":\"network\","
    ...
    out << "{\"proto_version\":1,\"request_id\":\"" << JsonEscape(requestId) << "\","
        ...
        << "\"queue_dropped\":" << logger_.GetDroppedCount()
        << extra
        << "}";
    return out.str();
}
