...
IpcControlServer::IpcControlServer(UsbMonitor& monitor, std::function<void()> shutdownCallback)
...
std::string IpcControlServer::Dispatch(const std::string& requestJson, void* pipeHandle) {
    ...
    if (isMutating) {
        ...
        if (ExtractJsonString(requestJson, "expected_session_id", expectedSession) &&
            expectedSession != UsbLogger::GetSessionId()) {
            ok = false;
    ...
    if (staleRejected) {
        ...
        if (!ExtractJsonBool(requestJson, "enabled", enabled)) {
            ...
            UsbLogger::SetSaveLogsEnabled(enabled);
            revision_.fetch_add(1, std::memory_order_relaxed);
        }
    }
        ...
        if (!ExtractJsonUint64(requestJson, "budget_bytes", budgetBytes) || budgetBytes == 0) {
            ...
            const uint64_t fileBytes = static_cast<uint64_t>(UsbLogger::GetMaxFileBytes());
            ...
            UsbLogger::SetRetentionMaxArchives(archives);
            revision_.fetch_add(1, std::memory_order_relaxed);
        ...
        auto lines = UsbLogger::GetRecentLines();
    ...
    if (isMutating) {
        ...
        audit << "{\"t_unix_ms\":" << now_ms << ",\"endpoint\":\"usb_monitor\","
    ...
    out << "{\"proto_version\":1,\"request_id\":\"" << JsonEscape(requestId) << "\","
        ...
        << "\"endpoint_id\":\"port\","
        << "\"session_id\":\"" << UsbLogger::GetSessionId() << "\","
        << "\"revision\":" << revision_.load() << ","
        << "\"monitoring_enabled\":" << (monitor_.IsMonitoringEnabled() ? "true" : "false") << ","
        << "\"save_logs_enabled\":" << (UsbLogger::IsSaveLogsEnabled() ? "true" : "false")
        << extra
        << "}";
    return out.str();
}
