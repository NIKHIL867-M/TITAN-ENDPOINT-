// resource_pressure.h
//
// RAM + disk auto-lightening, shared pattern duplicated across all 5
// endpoints (no shared library between these independent CMake projects --
// same duplication convention already used for collector_health).
//
...
// endpoint's log directory (GetDiskFreeSpaceExA), and tracks the current
...
public:
    explicit ResourcePressureMonitor(std::string path_on_volume)
    ...
    void Update()
    {
        ...
        if (!path_.empty() &&
            GetDiskFreeSpaceExA(path_.c_str(), &free_bytes, nullptr, nullptr))
